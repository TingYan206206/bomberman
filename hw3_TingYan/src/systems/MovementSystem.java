...
public class MovementSystem extends GameSystem implements GameObjectListener, UpdateListener
{
	...
	public void update(UpdateEvent e)
	{
		for(Character c : m_characters)
		{
			...
			// if character get powerupfire, increase bombrange, the highest range is 4.
			if (m_board.currentAnim(y/ m_board.TILE_SIZE, x/ m_board.TILE_SIZE) == "powerUpFire")
			{
				int range = c.getM_bombRange();
				if (range < 5){
					range++;
					c.setM_bombRange(range);
				}
				m_board.currentAnim(y/ m_board.TILE_SIZE, x/ m_board.TILE_SIZE, "empty");
			}

			// if character get powerUpBomb, increase bomb number
			if (m_board.currentAnim(y/m_board.TILE_SIZE, x/m_board.TILE_SIZE) == "powerUpBomb")
			{
				int num = c.getM_bombNum();
				c.setM_bombNum(++num);
				m_board.currentAnim(y/m_board.TILE_SIZE, x/m_board.TILE_SIZE, "empty");
			}

			// if character get powerupSpeed, increase speed, the highest speed is 5.
			if (m_board.currentAnim(y/m_board.TILE_SIZE, x/m_board.TILE_SIZE) == "powerUpSpeed")
			{
				int speed = c.getM_speed();
				if (speed < 6 ) {
					speed++;
					c.setM_speed(speed);
				}
